...
function generateFieldLines(sceneId, P, field) {
  ...
  switch (sceneId) {
    ...
    case 'loop': {
      ...
      const planeCount = ({'清晰': 2, '标准': 4, '丰富': 6})[P.density] || 6;
      const baseLines = [];
      {
        const a = baseAngle;
        ...
        for (const radialSign of [-1, 1]) {
          for (const sd of allSeeds) {
            ...
            const res = runTrace(s, { dir: 1, maxSteps: sd.max, bounds: Math.max(11, 5.5 * R), closeTol: sd.closeTol, step: sd.step, planeN });
            if (res.closed && res.pts.length > 20) baseLines.push(res.pts);
          }
        }
      }
      // One group is a complete meridian plane, including both radial sides.
      // A pi interval contains each plane once; rotation preserves the loop field.
      const axis = new THREE.Vector3(0, 1, 0);
      for (let k = 0; k < planeCount; k++) {
        for (const pts of baseLines) push(pts.map(p => p.clone().applyAxisAngle(axis, k * Math.PI / planeCount)), true);
      }
      traceStats.planeCount = planeCount;
      traceStats.linesPerPlane = baseLines.length;
      break;
    }
    case 'solenoid': {
      ...
      const yAxis = new THREE.Vector3(0, 1, 0);
      for (const delta of [-0.38, 0, 0.38, Math.PI - 0.38, Math.PI, Math.PI + 0.38]) {
...
function computeSectionBasis(section = SECTION) {
  let n;
...
export { PAL, HEAT_RES, FL_STEP, MU0, LENGTH_UNIT_M, TESLA_TO_MICROTESLA, BIOT_SAVART_SCALE, MAGNETIC_POLE_SCALE, WIRE_RADIUS, LOOP_WIRE_RADIUS, GAUSS48_NODES, GAUSS48_WEIGHTS, evalMonopoles, evalRectFace, evalDipole, evalWire, buildRingSegs, evalSegs, carlsonRF, carlsonRD, ellipticKE, evalCircularLoopY, evalPolylineField, pointSegmentDistanceSq, buildBentWirePath, appendRectFace, SCENES, EARTH_R, buildField, traceLine, orientAlongB, decimateLine, assessLineQuality, generateFieldLines, SECTION, computeSectionBasis, traceProjectedLine, generateProjectedStreamlines, heatColor, sampleModel, runPhysicsValidation, runGeometryValidation };
